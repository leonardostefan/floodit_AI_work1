...
FieldList *reduceGraph(FieldList *b)
{
    ...
    FieldNode **newList = calloc(b->realSize, sizeof(FieldNode *));
    int *newId = calloc(b->size, sizeof(int));
    int newIdPos = 0;
    for (int id = 0; id < b->size; id++)
    {
        ...
        {
            newId[id] = newIdPos;
            newList[newIdPos] = b->list[id];
            newList[newIdPos]->id = newIdPos;
            newIdPos++;
        }
    }
    for (int id = 0; id < newIdPos; id++)
    {
        for (int n = 0; n < newList[id]->neighborsSize; n++)
        {
            int nId = newList[id]->neighborsId[n];
            if (newId[nId] > newIdPos)
            {
                breakDebug();
            }
            newList[id]->neighborsId[n] = newId[nId];
        }
    }

    newBoard->list = newList;
    newBoard->realSize = newIdPos;
    newBoard->size = newIdPos;
